  ...
  <form>
    <div class="space-y-12 mx-8 my-4">
      <div class="border-b border-gray-900/10 pb-12">
        <h2 class="text-base/7 font-semibold text-gray-900">{{ translator.trans('page.job_detail.application_form.title') }}</h2>

        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
          <div class="sm:col-span-4">
            <label for="name" class="block text-sm/6 font-medium text-gray-900">{{ translator.trans('page.job_detail.application_form.name') }}</label>
            <div class="mt-2">
              ...
              <p v-if="errors.name" class="mt-2 text-sm/6 text-red-600">{{ errors.name }}</p>
            </div>
          </div>

          <div class="sm:col-span-4">
            <label for="email" class="block text-sm/6 font-medium text-gray-900">{{ translator.trans('page.job_detail.application_form.email') }}</label>
            <div class="mt-2">
              <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
              ...
              <p v-if="errors.email" class="mt-2 text-sm/6 text-red-600">{{ errors.email }}</p>
            </div>
          </div>

          <div class="col-span-full">
            <label for="about" class="block text-sm/6 font-medium text-gray-900">{{ translator.trans('page.job_detail.application_form.cover_letter') }}</label>
            <div class="mt-2">
            ...
            <p v-if="errors.coverLetter" class="mt-2 text-sm/6 text-red-600">{{ errors.coverLetter }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  try {
    ...
    if (error.response && error.response.status === 422) {
      error.response.data.violations.forEach((violation: any) => {
        errors.value[violation.propertyPath] = violation.title;
      });
    }
  } finally {
